...
q1:
	@bash scripts/q1_audit.sh

# q1-simple: Quick check — verifies Docker image builds and binary is present
q1-s:
	@bash scripts/q1_simple.sh
...
q12:
	@ls tests/*.rs > /dev/null 2>&1 && echo "[PASS] test files exist" || { echo "[FAIL] no test files"; exit 1; }
...
help:
	...
	@echo "  q1   Full Zone 01 audit: build zone-filler, run bender vs terminator + student vs bender"
	@echo "  q1-s Simple Docker image check: verifies image builds and binary is present"
